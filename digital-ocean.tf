...
resource "digitalocean_droplet" "http3_yurets_pro" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sleep 60; sudo apt-get -y update && sudo DEBIAN_FRONTEND=noninteractive apt-get -y install htop nload iotop net-tools docker.io",
      ...
      "/usr/bin/docker run --privileged --name nginx -d -p 80:80 -p 443:443/tcp -p 443:443/udp -v /opt/letsencrypt/:/opt/nginx/certs/ -v /opt/nginx.conf:/etc/nginx/nginx.conf -v /opt/index.html:/etc/nginx/html/index.html ymuski/nginx-http3"
    ]
  }
